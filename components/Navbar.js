...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── Vision Insurance Bar ─────────────────────────────────── */}
      {insuranceVisible && (
        <div className="bg-[#f5f0e8] border-b border-[#e5d9c3] text-center py-2 px-4 text-xs font-medium text-[#5c4a2a] relative z-50">
          <span>
            Now accepting vision insurance.{' '}
            <a href="/contact" className="font-bold underline underline-offset-2 hover:text-[#3d3019] transition-colors">
              Learn More
            </a>
          </span>
          <button
            onClick={() => setInsuranceVisible(false)}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5c4a2a]/60 hover:text-[#5c4a2a] transition-colors text-base leading-none cursor-pointer"
            aria-label="Close"
          >
            ×
          </button>
      ...
      {saleBannerVisible && (
        <div className="bg-white border-b border-border relative z-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-4 py-3">

              {/* Left: Label + Offer + Code */}
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 min-w-0">
                <span className="text-[10px] font-black tracking-widest uppercase text-red-600 shrink-0">
                  🏷️ Summer Sale
                </span>
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 min-w-0">
                  <p className="text-sm sm:text-base font-black text-charcoal leading-tight">
                    Buy One Get 2nd <span className="text-accent">65% off</span> + One Free Case*
                  </p>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-ebd-blue-light border border-accent/20 text-accent text-xs font-black tracking-widest shrink-0">
                    CODE: <span className="font-black">OPTIC2GOOD</span>
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 shrink-0">
                <div className="hidden sm:flex flex-col items-end">
                  <a
                    href="/products"
                    className="px-4 py-2 rounded-lg bg-accent text-white text-xs font-bold hover:bg-accent-dark transition-colors whitespace-nowrap shadow-sm"
                  >
                    Save today
                  </a>
                  <span className="text-[9px] text-text-muted mt-0.5">Exclusions Apply</span>
                </div>
                <button
                  onClick={() => setSaleBannerVisible(false)}
                  className="w-6 h-6 rounded-full flex items-center justify-center text-text-muted hover:text-charcoal hover:bg-light-gray transition-colors cursor-pointer text-sm"
      ...
      {promoVisible && (
        <div className="bg-gradient-to-r from-ebd-blue to-ebd-teal text-white text-center py-2 px-4 text-xs font-semibold tracking-wide relative z-50">
          <span>🚚 Free Shipping on orders over $99 · Use code <strong>OPTIC20</strong> for 20% off your first order</span>
          <button
            onClick={() => setPromoVisible(false)}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white transition-colors text-base leading-none cursor-pointer"
