...
const generateBlueLightGlasses = () => {
  const products = [];
  const brands = ['Felix Gray', 'Warby Parker', 'Quay', 'Prive Revaux', 'EyeBuyDirect', 'Zenni', 'Gunnar'];
  const frameShapes = ['Round', 'Square', 'Rectangle', 'Oval', 'Cat-Eye', 'Geometric'];
  const frameColors = ['Black', 'Tortoise', 'Clear', 'Blue', 'Pink', 'Rose Gold', 'Gold', 'Silver'];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const price = Math.floor(Math.random() * 80) + 40 + 0.99; // 40.99 to 120.99
    
    products.push({
      id: `bluelight-${Date.now()}-${i}`,
      name: `${brand} ${color} ${shape} Blue Light ${i+1}`,
      brand: brand,
      price: price,
      ...
      description: `Premium blue light blocking glasses featuring a comfortable ${material.toLowerCase()} frame and stylish ${shape.toLowerCase()} design. Protect your eyes from digital screen glare.`
    });
  }
  return products;
};
...
// Inject Products
const newProducts = generateBlueLightGlasses();
const productsJSON = newProducts.map(p => JSON.stringify(p, null, 2)).join(',\n  ');

let newContent = content.replace(/export const products = \[/, `export const products = [\n  ${productsJSON},`);

fs.writeFileSync('data/siteData.js', newContent);
console.log('Appended 25 blue light products to siteData.js');
