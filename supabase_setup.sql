...
-- Drop existing tables to ensure schema type changes apply cleanly
...
CREATE POLICY "Users can view own prescriptions" ON public.prescriptions 
  FOR SELECT USING (auth.uid() = user_id OR ((auth.jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'admin'::text);
CREATE POLICY "Users can update own prescriptions" ON public.prescriptions 
  FOR UPDATE USING (auth.uid() = user_id OR ((auth.jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'admin'::text);
CREATE POLICY "Users can delete own prescriptions" ON public.prescriptions 
  FOR DELETE USING (auth.uid() = user_id OR ((auth.jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'admin'::text);
